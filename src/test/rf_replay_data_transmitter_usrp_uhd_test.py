...
def run_replay_data_transmitter(
    graph,
    replay,
    ports_to_test=None,
    num_bytes=None,
    ...
    tdms_file = TdmsFile.read(
        "/home/agaber/workarea/lti-6g-sw/data-recording-tools/LabVIEW/TDMS Waveform Files/NR_FR1_DL_FDD_SISO_BW-20MHz_CC-1_SCS-30kHz_Mod-64QAM-OFDM-TM3.1.tdms"
    )
    # get all channels
    ...
    # input_data = [tx_data[0:num_words]]
    ...
    print(output_data)  # for debug

    ## Figure out how many bytes to send
    word_size = replay.get_word_size()
    print("Word size: ", word_size)
    max_packet_size = replay.get_max_packet_size(0)
    print("Max Packet size: ", max_packet_size)
    max_items_per_packet = replay.get_max_items_per_packet(0)
    print("Max items per packet: ", max_items_per_packet)
    play_offset = replay.get_play_offset(0)
    print("play_offset: ", play_offset)
    play_size = replay.get_play_size(0)
    print("play_size: ", play_size)
    play_type = replay.get_play_type(0)
    print("play_type: ", play_type)
    get_record_fullness = replay.get_record_fullness(0)
    print("get_record_fullness: ", get_record_fullness)
    get_record_item_size = replay.get_record_item_size(0)
    print("get_record_item_size: ", get_record_item_size)
    get_record_size = replay.get_record_size(0)
    print("get_record_size: ", get_record_size)
    get_record_type = replay.get_record_type(0)
    print("get_record_type: ", get_record_type)
    ...
    for test_idx in range(num_tests):
        ...
        if use_tqdm:
            ...
            print("num pakets", num_pkts)  # for debug
