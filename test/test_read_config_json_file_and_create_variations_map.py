...
class RFDataRecorderAPI:
        ...
        def __init__(self,config):
            ...
            for index, value in enumerate (variations):
                ...
                if parameter_config["SeqType"] == "range":
                    ...
                    parameter_values_list= list(RFDataRecorderAPI.drange(parameter_values_range[0],parameter_values_range[1],parameter_values_range[2]))
            ...
            variations_product= DataFrame({parameter_config["Parameter"]:parameter_config["Values"]}) 
            for i in range(len(variations)-1):
                parameter_config=variations[i+1]
                data_frame_i= DataFrame({parameter_config["Parameter"]:parameter_config["Values"]}) 
            ...
            general_config_dic= DataFrame({parameter_config["Parameter"]:[parameter_config["Value"]]}) 
            for i in range(len(general_config)-1):
                parameter_config=general_config[i+1]
                data_frame_i= DataFrame({parameter_config["Parameter"]:[parameter_config["Value"]]}) 
    ...
    class RxRFDataRecorderConfig:
        ...
        def __init__(self,iteration_config,iteration_general_config):
            ...
            self.output_path = (Path(__file__).parent / iteration_general_config["rx_recorded_data_path"]).resolve() 
    ...
    def calculate_master_clocl_rate(data_recording_api_config):
        ...
        data_recording_api_config.args= data_recording_api_config.args+",master_clock_rate=184.32e6"
...
def main():
    ...
    for i in range(len(variations_map.variations_product)):
        ...
        tx_data_recording_api_config = RFDataRecorderAPI.TxRFDataRecorderConfig(iteration_config,iteration_general_config)
        rx_data_recording_api_config = RFDataRecorderAPI.RxRFDataRecorderConfig(iteration_config,iteration_general_config)

        # calculate USRP master clockrate based on given rate
        tx_data_recording_api_config = RFDataRecorderAPI.calculate_master_clocl_rate(tx_data_recording_api_config)
        rx_data_recording_api_config = RFDataRecorderAPI.calculate_master_clocl_rate(rx_data_recording_api_config)
